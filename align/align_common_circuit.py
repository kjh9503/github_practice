...
def load_circuits(circuit_path, num_head):
    info = torch.load(circuit_path)['path']
    circuit = {}
    for layer_idx, heads in info :
        circuit[layer_idx] = heads[:num_head]
    # circuit_list = [(layer, head) for layer, heads in circuits for head in heads]
...
triple_embeddings_1 = torch.load('./data/triples_txt_embed_t1_llm_t1_chain_gpt.pt').to('cuda')
triple_embeddings_2 = torch.load(f'./data/triples_txt_embed_{args.time}_llm_t1_chain_gpt.pt').to('cuda')
...
ent_emb_1 = torch.load('./data/entity_embedding_t1.pt').to('cuda')
rel_emb_1 = torch.load('./data/relation_embedding_t1.pt').to('cuda')

ent_emb_2 = torch.load(f'./data/entity_embedding_{args.time}.pt').to('cuda')
rel_emb_2 = torch.load(f'./data/relation_embedding_{args.time}.pt').to('cuda')
...
for i, triple in enumerate(train_2):
    ...
    if h in names:
        hr2t_idx_t2[(h, r)].append((t, i))  # list in case of duplicates
...
for i, triple in enumerate(train_1):
    ...
    if h in names and (h, r) in hr2t_idx_t2:
        # T2에서 매칭되는 t 하나 뽑음 (여러 개일 경우 첫 번째)
...
circuits = dict(list(load_circuits(args.circuit_path, args.num_circuit_head).items())[:args.num_circuit_layer])
print(circuits)
...
# align_model = torch.nn.DataParallel(align_module)
align_module.to(DEVICE)
optimizer = optim.Adam(align_module.parameters(), lr=LR)

# optimizer2 = optim.Adam(model.parameters(), lr=0)
...
if args.locality_test :
    if 'chain' in args.model_name :
        dir_name = f'log/batch_Common_Circuit(L{args.num_circuit_layer}H{args.num_circuit_head})_{args.time}_essence_{args.kl_factor}_decay_{args.decay_factor}_locality_L{ATTN_LAYER_INDEX}_N{num_edit}_E{epoch}_{args.note}'
    else :
        dir_name = f'log/batch_Common_Circuit(L{args.num_circuit_layer}H{args.num_circuit_head})_{args.time}_essence_{args.kl_factor}_decay_{args.decay_factor}_locality_L{ATTN_LAYER_INDEX}_N{num_edit}_E{epoch}_{args.note}'
else :
    raise NotImplementedError('CONDUCT LOCALITY TEST')
...
for batch_num in range(total_batches):
    ...
    align_module = AlignModule_Lite_Circuit(ent2id, rel2id).to('cuda')
    ...
    for e in range(epoch):
        ...
        triples_idx_1 = torch.tensor([person_triples_idx_1[i] for i in batch_indices]).to('cuda')
        triples_idx_2 = torch.tensor([person_triples_idx_2[i] for i in batch_indices]).to('cuda') #(16, 3)
        head_ids = triples_idx_2[:, 0].tolist()

        subg_txt_repr_pt_1 = torch.stack([subg_txt_repr_ind_1[hid] for hid in head_ids]).to('cuda') #(16, 4096)
        subg_txt_repr_pt_2 = torch.stack([subg_txt_repr_ind_2[hid] for hid in head_ids]).to('cuda')

        text_embedding_1 = torch.stack([triple_embeddings_1[i] for i in batch_indices]).to('cuda') #(16, 3, 4096)
        text_embedding_2 = torch.stack([triple_embeddings_2[i] for i in batch_indices]).to('cuda')


        texts_2 = [f"{person_triples_txt_2[i][0]} {person_triples_txt_2[i][1]} {person_triples_txt_2[i][2]}" for i in batch_indices]


        input_ids_2 = tokenizer(texts_2, return_tensors="pt", max_length=25, padding='max_length', truncation=True).to('cuda')
    ...
    input_ids_hr = tokenizer(hr_texts, return_tensors="pt", padding=True, truncation=True).to('cuda')
    ...
    if args.locality_test :
        def batch_generate_all(batch_size=32, max_new_tokens=20):
            ...
            for b in trange(total_batches, desc='Locality Test'):
                ...
                batch_idx = torch.LongTensor(locality_triple_inds[batch_start:batch_end]).to('cuda')


                text_embedding = triple_embeddings_loc[batch_start:batch_end].to('cuda')


                head_ids = batch_idx[:, 0].tolist()
                subg_txt_repr_pt_1_loc = torch.stack([subg_txt_repr_ind_loc_1[hid] for hid in head_ids]).to('cuda')
                subg_txt_repr_pt_2_loc = torch.stack([subg_txt_repr_ind_loc_2[hid] for hid in head_ids]).to('cuda')
